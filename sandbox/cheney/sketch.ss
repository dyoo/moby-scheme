...
;; change-direction: world key -> world
;; Handles changes in direction due to key press.
(define (change-direction w k)
  (cond
    [(key=? k 'left)
     (update-world-direction w "left")]
    [(key=? k 'right)
     (update-world-direction w "right")]
    [(key=? k 'up)
     (update-world-direction w "up")]
    [(key=? k 'down)
     (update-world-direction w "down")]
    [else
     w]))
