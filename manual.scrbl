...
program running from the web page.  The page should also provide links
...
@subsection{Bindings from ISL}

The following toplevel bindings are available from Moby, and have the
same meaning as in @hyperlink["http://docs.plt-scheme.org/htdp-langs/intermediate-lambda-prim-op.html"]{Intermediate Student Language}.
...
@subsection{Extensions to ISL}
 
Moby also provides the following forms from ASL:
@(itemlist
  @item{@scheme[begin]}
  @item{@scheme[set!]}
  @item{@scheme[let]}
  ...
  @item{@scheme[letrec]}
  @item{@scheme[case]}
  @item{@scheme[build-vector]}
  @item{@scheme[make-vector]}
  @item{@scheme[vector]}
  @item{@scheme[vector-length]}
  @item{@scheme[vector-ref]}
  @item{@scheme[vector-set!]}
  @item{@scheme[vector?]}
  )
...
Does an HTTP GET to grab the string content from a URL.

As an example, @schemeblock[
...
@defproc[(parse-xml (xml-string string?)) s-expression?]{
...
(schemeblock
(js-big-bang 0   
             ;; The initial value of the world is 0,
             ;; and every second that passes triggers an increment.
             (on-tick 1 add1)))
...
example of a handler is @scheme[on-tick], which registers a function to update
...
The following program shows an expanding circle.
...
the given stimulus is fired off, the old world is used to compute both
...
Delays by n seconds, and then fires the handlers.}
...
@itemlist[
    ...
    @item{Grab the Moby source, currently hosted on 
